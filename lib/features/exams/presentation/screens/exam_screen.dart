...
class _ExamScreenState extends State<ExamScreen> {
  ...
  /// Value of the "All ..." row in every filter.
  static const int allValue = -1;

  final TextEditingController searchController = TextEditingController();

  /// Exam list from getAllExams. Null selection means "All exams".
  List<GetAllExamData> examOptions = [];

  /// True while getAllExams is in flight, so the exam field shows the same
  /// spinner the Select Exam screen shows.
  ...
  void initState() {
    ...
    final List<TutorshipClass> standardList = standardOptions;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Fills the exam filter. Independent of the mark entry call, so one
  ...
  // ---------------------------------------------------------------------
  // Class data. Cached from the class details API at login, so the
  // standard / division / subject filters need no call of their own.
  // ---------------------------------------------------------------------

  /// Both cached lists, searched together. A standard can appear more than
  /// once - the class list carries one entry per standard/division pair -
  /// so every entry has to be visited, not just the first match.
  List<TutorshipClass> get classSources => [
    ...AppData.standards,
    ...AppData.tutorshipClasses,
  ];

  /// One row per standard.
  List<TutorshipClass> get standardOptions {
    final Map<int, TutorshipClass> unique = {};

    for (final TutorshipClass item in classSources) {
  ...
  /// Divisions of [standardId], merged across every entry carrying that
  /// standard. A null standard means every division there is.
  List<DivisionDetails> divisionsFor(int? standardId) {
    final Map<int, DivisionDetails> unique = {};

    for (final TutorshipClass item in classSources) {
      if (standardId != null && item.standardId != standardId) {
        continue;
      }
  ...
  /// Subjects under [standardId] / [divisionId], merged the same way. A
  /// null level means "any", so the list widens instead of going empty.
  List<SubjectDetails> subjectsFor(int? standardId, int? divisionId) {
    ...
    for (final TutorshipClass item in classSources) {
      ...
      for (final DivisionDetails division
          in item.division ?? const <DivisionDetails>[]) {
        if (divisionId != null && division.divisionId != divisionId) {
          continue;
        }
  ...
  Future<void> _fetchExams() async {
    ...
    debugPrint('ExamTermId: $selectedExamTermId');
    debugPrint('ExamTypeId: $selectedExamTypeId');
    debugPrint('ExamId (local filter): $selectedExamId');
  ...
  List<MarkEntryEntity> get filteredExams {
    ...
    return exams.where((exam) {
      ...
      final bool matchesDivision =
          selectedDivisionId == null || exam.divisionId == selectedDivisionId;

      final bool matchesSubject =
          selectedSubjectId == null || exam.subjectId == selectedSubjectId;

      // The mark entry API takes a term and a type, not an exam id, so
      // the exact exam is matched here on the list it returned.
      final bool matchesExam =
          selectedExamId == null || exam.examId == selectedExamId;
          ...
          Expanded(
            child: BlocConsumer<ExamCubit, ExamState>(
              listener: (context, state) async {
                ...
                if (state is GetAllExamLoading) {
                  setState(() {
                    isExamLoading = true;
                  });
                }

                // Fills the exam filter. The selection is left on
                // "All exams" so the list is not narrowed on open.
                if (state is GetAllExamSuccess) {
                  setState(() {
                    isExamLoading = false;

                    examOptions = state.response.data ?? [];

                    final bool selectionExists = examOptions.any(
  ...
  /// Shared shape for the four filters: an "All ..." row on top, the
  /// option sheet, and the field itself.
  Widget _buildFilterDropdown({
    required String hint,
    required String iconAsset,
    required String allLabel,
    required String sheetTitle,
    required List<DropdownMenuItem<int>> optionItems,
    ...
    final int pickerValue = selectedId ?? allValue;
    ...
    return GestureDetector(
      ...
      onTap: isLoading
          ...
          : () async {
              ...
              final int value = result.value!;

              await onSelected(value == allValue ? null : value);
            },
      child: InputDecorator(
        decoration: _standardDecoration(iconAsset: iconAsset),
        child: Row(
          children: [
            Expanded(
              child:
                  ...
                  Text(
                    isLoading
                        ? 'Loading...'
                        : (selectedName?.trim().isNotEmpty == true
                              ? selectedName!.trim()
                              : hint),
  ...
  /// Exam filter, from the getAllExams response.
  Widget _buildExamDropdown() {
    final List<DropdownMenuItem<int>> optionItems = examOptions
        .where((item) => item.examId != null)
        .map((item) {
    ...
    return _buildFilterDropdown(
      hint: 'Exam',
      allLabel: 'All exams',
      sheetTitle: 'Select Exam',
      optionItems: optionItems,
      selectedId: selectedExamId,
      selectedName: selectedExamName,
      isLoading: isExamLoading,
      iconAsset: 'assets/icons/Group (18).svg',
      onSelected: (value) async {
        GetAllExamData? selectedExam;

        for (final GetAllExamData item in examOptions) {
        ...
        setState(() {
          ...
          selectedExamName = selectedExam?.examName;

          // The request carries term and type; the exam id narrows the
          // returned list to the one exam.
          selectedExamTermId = selectedExam?.examTermId;
          selectedExamTypeId = selectedExam?.examTypeId;
        });
  ...
  /// Standard filter, from the cached class details.
  // Widget _buildStandardDropdown() {
  //   final List<DropdownMenuItem<int>> optionItems = standardOptions.map((item) {
  //     return DropdownMenuItem<int>(
  ...
  //     sheetTitle: 'Select Standard',
  //     optionItems: optionItems,
  //     selectedId: selectedStandardId,
  //     selectedName: selectedStandardName,
  //     onSelected: (value) async {
  //       TutorshipClass? selectedStandard;

  //       for (final TutorshipClass item in standardOptions) {
  ...
  //         // A new standard invalidates whatever division and subject
  //         // were chosen under the old one.
  ...
  Widget _buildStandardDropdown() {
    ...
    return _buildFilterDropdown(
      hint: 'All standards', // 👈 initial display
      allLabel: 'All standards',
      sheetTitle: 'Select Standard',
      optionItems: optionItems,
      selectedId: selectedStandardId,
      selectedName: selectedStandardName,
      iconAsset: 'assets/icons/Group 951.svg',
      onSelected: (value) async {
        TutorshipClass? selectedStandard;
  ...
  /// Division filter, narrowed to the selected standard.
  Widget _buildDivisionDropdown() {
    final List<DivisionDetails> divisions = divisionsFor(selectedStandardId);

    final List<DropdownMenuItem<int>> optionItems = divisions
    ...
    return _buildFilterDropdown(
      ...
      onSelected: (value) async {
        ...
        setState(() {
          ...
          selectedDivisionName = selectedDivision?.division;
  ...
  /// Subject filter, narrowed to the selected standard and division.
  Widget _buildSubjectDropdown() {
    final List<SubjectDetails> subjects = subjectsFor(
      selectedStandardId,
      selectedDivisionId,
    );

    final List<DropdownMenuItem<int>> optionItems = subjects
  ...
  InputDecoration _standardDecoration({required String iconAsset}) {
