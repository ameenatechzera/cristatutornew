    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ButtonStyle(
          ...
          foregroundColor: const WidgetStatePropertyAll(Color(0xff333333)),
...
//           ),
//           alignment: Alignment.center,
//           child: Icon(icon, color: const Color(0xff555555), size: 15),
//         ),
//       ),
//     );
//   }
...
class _DiaryFilterDrawerState extends State<DiaryFilterDrawer> {
  ...
  /// Every class in the school, so the filter is not limited to the
  /// teacher's own tutorship classes. Falls back to the fetched list when
  /// AppData has nothing.
  List<TutorshipClass> get allClasses {
    return AppData.standards.isNotEmpty ? AppData.standards : tutorshipClasses;
  }

  /// Divisions of [standard]. The complete Standard list can carry no
  ...
  List<DivisionDetails> _divisionsFor(TutorshipClass standard) {
    final List<DivisionDetails> own = standard.division ?? [];
  ...
  void initState() {
    ...
    if (allClasses.isEmpty) {
      ...
      _setInitialSelection(allClasses);
    }
  }
  ...
  /// selection can take over.
  void _restoreSelectionNames() {
    if (allClasses.isEmpty || selectedStandardId == null) {
      return;
    }

    final options = _getClassOptions(allClasses);
    final match = _getSelectedClassOption(options);
  ...
  void _setInitialSelection(List<TutorshipClass> classes) {
    ...
    for (final standard in classes) {
      final divisions = _divisionsFor(standard);
  ...
  List<_ClassOption> _getClassOptions(List<TutorshipClass> classes) {
    ...
    // Guards against the same Standard + Division arriving from both
    // AppData.standards and the tutorship fallback.
    ...
    for (final standard in classes) {
      ...
      for (final division in divisions) {
        if (!added.add('${standard.standardId}-${division.divisionId}')) {
          continue;
        }
                ...
                child: BlocConsumer<AuthenticationCubit, AuthenticationState>(
                  ...
                  builder: (context, state) {
                    ...
                    if (allClasses.isNotEmpty) {
                      return _buildDynamicFilterContent(allClasses);
                    }
