...
class SelectYourClassScreen extends StatefulWidget {
  ...
  /// Diary category sent to the save API, for example "Homework".
...
class _SelectYourClassScreenState extends State<SelectYourClassScreen> {
  ...
  /// True when a voice recording is finished and still attached.
  bool get _hasRecording {
    return voiceState == VoiceState.recorded &&
        _recordedAudioPath != null &&
        _recordedAudioPath!.isNotEmpty;
  }

  /// Anything at all on the form. A diary can be saved with only one of
  /// these: a title, a description, an attachment or a recording.
  ...
  void initState() {
    ...
    debugPrint('Due Date    : ${formatApiDate(widget.dueDate)}');
    debugPrint('Is Favourite: ${widget.isFavourite}');
    debugPrint('Diary Type   : ${widget.diaryType}');
    debugPrint('Employee ID : ${AppData.employeeId}');
    debugPrint('==========================================');
  }
  ...
  Future<void> _saveDiary() async {
    ...
    // Title, description, attachments and the recording are all optional
    // on their own — only an completely empty diary is refused.
    if (!_hasAnyContent) {
      _showMessage(
        'Please add a title, description, attachment or voice recording',
      );
      return;
    }
    ...
    // Every formatted run travels as its own inline span. Empty fields stay
    // empty strings, so a diary can carry only an attachment or a voice
    // recording.
    final String styledTitle = title.isEmpty ? '' : _titleController.toHtml();

    final String styledDescription = description.isEmpty
        ? ''
        : _descriptionController.toHtml();
